...
% TIMIT_SE - speech enhancement with objectice scoring using [1] and [2].
%
% Inputs:
...
%% BRIEF:          Speech enhancement with objective scoring using [1] and [2].
%
%% REFERENCE:
...
for i=1:length(Q)
    ...
    for j=1:length(x.files)
        ...
        %% IBM ESTIMATE SE
        pp.ne = 1; % use SPP noise estimation [1].
        pp.lg = 2; % perceptually motivated estimator from [2].
        x_hat.wav = ssubmmse(y.wav, fs, pp);
        SNR = segsnr(x.wav, x_hat.wav, fs); % find SegSNR and SNR.
        WPESQ = pesqbin(x.wav, x_hat.wav, fs, 'wb'); % find Wideband PESQ.
        QSTI = qsti(x.wav, x_hat.wav, fs); % find QSTI.
