...
% TIMIT_SE - speech enhancement with objective scoring using [1].
%
% Inputs:
...
%% BRIEF:          Speech enhancement with objective scoring using [1].
%
%% REFERENCE:
...
for i=1:length(Q)
    ...
    for j=1:length(x.files)
        ...
        %% MMSE-LSA RESULTS
        x_hat.wav = mmse(y.wav, fs, 0); % MMSE-LSA.
        if length(x_hat.wav) < length(y.wav)
            x_hat.wav = [x_hat.wav; x.wav(length(x_hat.wav)+1:end)];
        end
