function tf_timit_si(timit_path, noise_path, save_path, val_frac, Q)
% TF_TIMIT_SI - creates training, validation, and test speaker identification subsets from the TIMIT corpus for TensorFlow.
%
% Inputs:
...
%% BRIEF:          Creates training, validation, and test speaker identification subsets from the TIMIT corpus for TensorFlow.

%% FILE LISTS
d.files = dir(strcat(noise_path,'/*.wav')); % noise files.
train.files = [dir([timit_path,'/timit/*/*/*/si*.wav']); ...
    dir([timit_path,'/timit/*/*/*/sx*.wav'])]; % training files.
test.files = dir([timit_path,'/timit/*/*/*/sa*.wav']); % testing files.

%% RECORD INPUTS
fid = fopen(strcat('tf_timit_si_inputs.txt'), 'w');
fprintf(fid, 'TIMIT path: %s\nNoise path: %s\nSave path: %s\nValidation fraction: %g\nSNR values (dB): ', ...
    timit_path, noise_path, save_path, val_frac); % record paths.
