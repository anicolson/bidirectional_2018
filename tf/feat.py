...
def mag(x, Nw, Ns, NFFT):
    '''
    Computes the single-sided magnitude spectrum using the Hamming window.
    Includes the DC component (0), and the Nyquist frequency component (NFFT/2 + 1).
    ...
    Output: 
        MAG - single-sided magnitude spectrum.
    '''
...
def psd(x, Nw, Ns, NFFT, fs):
	...
	'''
	MAG = mag(x, Nw, Ns, NFFT) # single-sided magnitude spectrum.
	PSD = tf.div(tf.square(MAG), tf.to_float(Nw*fs)) # single-sided power spectral density.
...
def lsse(x, H, Nw, Ns, NFFT, fs):
    ...
    '''
    PSD = psd(x, Nw, Ns, NFFT, fs)
    #return tf.log(tf.matmul(PSD, H, False, True)) 
    LSSE = tf.log(tf.reshape(tf.matmul(tf.reshape(PSD, [tf.multiply(tf.shape(PSD)[0], 
        tf.shape(PSD)[1]), int(NFFT/2 + 1)]), H, False, True), [tf.shape(PSD)[0], 
        tf.shape(PSD)[1], H.shape[0]])) # log spectral subband energy.
...
def addnoisepad(x, d, x_len, d_len, Q, P, nconst):
	...
	Also normalizes the waveforms using nconst.
	...
	'''
	x = tf.div(tf.cast(tf.slice(x, [0], [x_len]), tf.float32), nconst) # remove padding and normalize.
	d = tf.div(tf.cast(tf.slice(d, [0], [d_len]), tf.float32), nconst) # remove padding and normalize.
