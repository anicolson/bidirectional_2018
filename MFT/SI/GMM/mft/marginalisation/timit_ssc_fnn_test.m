function timit_ssc_fnn_test(test_path, model_path, est_path, Tw, Ts, fs, Q)
% TIMIT_SSC_FNN_TEST - tests SSC-GMM speaker models using marginalization and an LSSE-FNN-IBM estimator for IBM estimation.
%
% Inputs:
...
%   Q - SNR values.
%
%% FILE:           timit_ssc_fnn_test.m 
%% DATE:           2017
%% AUTHOR:         Aaron Nicolson
%% AFFILIATION:    Signal Processing Laboratory, Griffith University
%% BRIEF:          Tests SSC-GMM speaker models using marginalization and an LSSE-FNN-IBM estimator for IBM estimation.
...
%% RECORD INPUTS
fid = fopen('timit_ssc_fnn_par.txt', 'w');
...
%% ASI TEST
load(model_path);
fid1 = fopen(strcat('timit_ssc_fnn_indi.txt'), 'w'); % individual test results.
fid2 = fopen(strcat('timit_ssc_fnn_acc.txt'), 'w'); % accuracy test results.
for i=1:length(Q)
    ...
    for j=1:length(x.files)
        ...
        %% LSSE-IBM ESTIMATE - LSSE-FNN-IBM ESTIMATOR
