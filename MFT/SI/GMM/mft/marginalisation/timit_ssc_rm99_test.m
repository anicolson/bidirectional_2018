function timit_ssc_rm99_test(test_path, model_path, Tw, Ts, fs, Q)
% TIMIT_SSC_RM99_TEST - tests SSC-GMM speaker models using marginalization and [1] for IBM estimation.
%
% Inputs:
...
%   Q - SNR values.
%
%% FILE:           timit_ssc_rm99_test.m 
%% DATE:           2018
%% AUTHOR:         Aaron Nicolson
%% AFFILIATION:    Signal Processing Laboratory, Griffith University
%% BRIEF:          Tests SSC-GMM speaker models using marginalization and [1] for IBM estimation.
%
%% REFERENCE:
%   [1] Raj, B., Seltzer, M.L. and Stern, R.M., 2004. Reconstruction of missing 
%       features for robust speech recognition. Speech communication, 43(4), 
%       pp.275-296
...
%% RECORD INPUTS
fid = fopen('timit_ssc_rm99_par.txt', 'w');
...
%% ASI TEST
load(model_path);
fid1 = fopen(strcat('timit_ssc_rm99_indi.txt'), 'w'); % individual test results.
fid2 = fopen(strcat('timit_ssc_rm99_acc.txt'), 'w'); % accuracy test results.
for i=1:length(Q)
    ...
    for j=1:length(x.files)
        ...
        %% LSSE-IBM ESTIMATE - RM99
        IBM = rm99(y.SSE, 5); % 5 frames used for noise estimation
