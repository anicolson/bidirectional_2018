...
# BRIEF:          Estimates the LSSE-IBM mask using a Bidirectional Recurrent Neural Network (BRNN).
...
## OPTIONS
training = True # perform training flag.
test = True # perform test flag.
save = True # save outputs.
version = 'LSTMCell-512-3' # model version.
cell_type = 'LSTMCell' # cell type.

## DATASETS
subset_path = '/home/aaron/aaron/home/aaron/datasets/tf_timit_si' # path to subsets.
noise_path = '/home/aaron/aaron/home/aaron/datasets/RSG-10/16KHz' # path to noise dataset.
save_path = '/home/aaron/feat/MFT/IBM/IBM_hat/BRNN/SI/TIMIT/LSSE/' + version # save path for output test files.
model_path = '/home/aaron/model/MFT/IBM/IBM_hat/BRNN/SI/TIMIT/LSSE/' + version # model save path.
...
H = feat.melfbank(M, int(NFFT/2 + 1), fs) # mel-scale filterbank.
...
cell_units = 512 # number of units per cell.
...
def feat_extr(x, d, x_len, d_len, Q, Nw, Ns, NFFT, fs, H, P, nconst):
	'''
    Computes LSSE input features, and the IBM target features. The
	...
	x_LSSE = feat.lsse(x, H, Nw, Ns, NFFT, fs) # clean LSSEs.
	d_LSSE = feat.lsse(d, H, Nw, Ns, NFFT, fs) # noise LSSEs.
	y_LSSE = feat.lsse(y, H, Nw, Ns, NFFT, fs) # noisy LSSEs.
	IBM = tf.to_float(tf.boolean_mask(tf.greater(x_LSSE, d_LSSE), 
		tf.sequence_mask(seq_len))) # Ideal Binary Mask (IBM).
	return (y_LSSE, IBM, seq_len)

## FEATURE GRAPH
print('Preparing graph...')
P = tf.reduce_max(x_len_ph) # padded waveform length.
feat = feat_extr(x_ph, d_ph, x_len_ph, d_len_ph, snr_ph, Nw, Ns, NFFT, 
	fs, H, P, nconst) # feature graph.
...
for i in range(0,hidden_layers):
	if i is 0:
		h.append(create.brnn_layer(feat[0], feat[2], cell_units, str(i+1), **brnn_opts))
	else:
		h.append(create.brnn_layer(h[i-1].output, feat[2], cell_units, str(i+1), **brnn_opts))

with tf.name_scope('mask'):
	mask = tf.boolean_mask(h[hidden_layers-1].output, tf.sequence_mask(feat[2]))
...
y_ = create.fc_layer(mask, h[hidden_layers-1].output_dim, num_outputs, 'out', **out_opts)
