function timit_se_mag_test(test_path, Tw, Ts, fs, Q)
% TIMIT_SE_MAG_TEST - tests MS-IBM estimation from [1] for the SE set.
%
% Inputs:
...
%	Q - SNR values.
%
%% FILE:           timit_si_mag_test.m
%% DATE:           2018
%% AUTHOR:         Aaron Nicolson
%% AFFILIATION:    Signal Processing Laboratory, Griffith University
%% BRIEF:          Tests magnitude spectrum IBM estimation from [1] for the SE set.
...
%% RECORD INPUTS
fid = fopen('timit_se_mag_par.txt', 'w');
...
%% NOISY TEST
fid1 = fopen(strcat('timit_se_mag_acc.txt'), 'w'); % individual test results.
for i=1:length(Q)
    ...
    for j=1:length(x.files)
        ...
        x = mag(x); % clean magnitude spectrum.
        d = mag(d); % noise magnitude spectrum.
        y = mag(y); % noisy magnitude spectrum.

        %% IBM
        ideal.IBM = x.MAG > d.MAG; % IBM with 0 dB threshold.   

        %% IBM ESTIMATE - RM99
        est.IBM = rm99(y.MAG, 5); % first 5 frames used for noise estimation.
